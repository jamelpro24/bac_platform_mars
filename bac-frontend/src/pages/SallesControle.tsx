...
export default function SallesControle() {
    ...
    setSessionRooms(prev => prev.map(r => {
      ...
      if (has) {
        ...
        const serieIns = getInscriptionsBySerie(serieId).filter(i =>
          isControle(i) && !takenControleNums.has(i.num_ins)
        );
  ...
  const selectedCount = new Set(sessionRooms.filter(r => r.salle_id && selectedRoomIds.has(r.salle_id)).map(r => r.salle_id)).size;

  const sortedTimeSlots = allTimeSlots;
  ...
  const availableForSession = inscriptions.filter(ins => {
    ...
    if (!isControle(ins)) return false;
    return true;
  });
    ...
    <div className="general" style={{ padding: "1.5rem 6cm 1.5rem 1.5rem", fontFamily: "'Cairo','Tajawal',sans-serif" }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        <div style={{ marginBottom: "1.5rem" }}>
          <h1 style={{ fontSize: 24, fontWeight: 700, color: "#1e466e", margin: 0 }}>قاعات مراقبة (التحكم)</h1>
          <p style={{ color: "#5f6368", fontSize: 13, margin: "4px 0 0" }}>إنشاء القاعات · توزيع المترشحين · وثائق المراقبة</p>
        </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
            ...
            <div style={c.card}>
              <div style={c.hd}>
                ...
                <h2 style={{ fontSize: 14, fontWeight: 600, color: "#202124", margin: 0 }}>اختر الدورة (مراقبة)</h2>
              </div>
            ...
            {selectedDate && (
              <div style={c.card}>
                <div style={c.hd}>
                  <div style={{ width: 26, height: 26, borderRadius: 8, background: "#fef3c7", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700, color: "#b45309" }}>4</div>
                  <h2 style={{ fontSize: 14, fontWeight: 600, color: "#202124", margin: 0, flex: 1 }}>توزيع القاعات (التحكم)</h2>
                  ...
                  {sessionRooms.length === 0 ? (
                    ...
                    <>
                      ...
                      <div style={{ display: "flex", gap: 8, marginTop: "1rem", justifyContent: "flex-end" }}>
                        <button onClick={saveDayAssignments} disabled={generating || sessionRooms.filter(r => r.salle_id).length === 0}
                          style={c.btn(generating ? "#9aa0a6" : "#1a56db")}>
                          <Save size={14} /> {generating ? "جاري الحفظ..." : "تسجيل التوزيع لليوم"}
                        </button>
                      </div>
                    </>
                  )}
                  ...
                  <>
                    <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: "1.25rem" }}>
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, borderBottom: "1px solid #f1f3f4", paddingBottom: 8 }}>
                        ...
                        <span style={{ fontWeight: 500, color: "#202124" }}>{sessionRooms.filter(r => r.salle_id).length} قاعة</span>
                      </div>
                    </div>
                    ...
                    {sessionRooms.filter(r => r.salle_id != null).length > 0 && (
